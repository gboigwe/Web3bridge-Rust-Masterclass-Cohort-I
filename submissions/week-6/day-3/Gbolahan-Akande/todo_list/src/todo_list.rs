use soroban_sdk::{
    contract, contractimpl, contracttype, symbol_short, xdr::SorobanCredentials, Env, String, Symbol, Vec,
};
...
impl Todolist {
    pub fn get_todos(env: &Env) -> Vec<Todo> {
        env.storage().persistent().get(&TODOS).unwrap_or(Vec::new(env))
    }
    pub fn create_todo(env: Env, title: String, description: String) -> Todo {
        let mut todos = Self::get_todos(&env);
        let mut new_id = env.storage().persistent().get(&NEXT_ID).unwrap_or(1);
        let todo = Todo {
            id: new_id,
            title,
            description,
        ...
        new_id += 1;
        env.storage().persistent().set(&NEXT_ID, &new_id);
        todo
    }
    ...
    pub fn complete(env: Env, id: u32) -> bool {
        let mut todos = Self::get_todos(&env);
    ...
    pub fn delete(env: Env, id: u32) -> bool {
        let mut todos = Self::get_todos(&env);
